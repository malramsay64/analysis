 ...
 */
    ...
    Vector2d cos(const Vector2d &v) {
        return Vector2d{std::cos(v.x), std::cos(v.y)};
    }

    Vector2d sin(const Vector2d &v) {
        return Vector2d{std::sin(v.x), std::sin(v.y)};
    }

    Vector2d tan(const Vector2d &v) {
        return Vector2d{std::tan(v.x), std::tan(v.y)};
    }
    ...
    Vector2d atan2(const Vector2d &vx, const Vector2d &vy) {
        return Vector2d{std::atan2(vx.x, vy.x), std::atan2(vx.y, vy.y)};
    }
